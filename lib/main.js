...
class Main {
  ...
  activate() {
    ...
    this.interval = atom.config.get('atom-slide-show.updateInterval') * 1000
    this.image_dir = atom.config.get('atom-slide-show.imageDir')
    ...
    if (atom.config.get('atom-slide-show.autoToggle')) {this.toggle()}
  }
  ...
  toggle() {
    if (this.toggled) {
      this.toggled = false
      if (atom.config.get('atom-slide-show.usePackageStyle')) {this.remove_style()}
      ...
      if (images.length > 0) {
        if (atom.config.get('atom-slide-show.usePackageStyle')) {this.preset_style()}
